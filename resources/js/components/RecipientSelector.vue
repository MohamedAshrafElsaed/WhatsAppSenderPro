    ...
    <div class="space-y-4">
        ...
        <div :class="isRTL() ? 'flex-row-reverse' : ''" class="flex gap-2">
            <div class="relative flex-1">
                <Search :class="isRTL() ? 'right-3' : 'left-3'" class="absolute top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                <Input
                    v-model="searchQuery"
                    :class="isRTL() ? 'pr-10' : 'pl-10'"
                    :placeholder="t('campaigns.search_contacts', 'Search contacts...')"
                />
            </div>
            <Button
                variant="outline"
        ...
        <div class="max-h-96 space-y-2 overflow-y-auto rounded-lg border p-4">
            <div
                ...
                :class="[
                    ...
                    isSelected(contact.id) ? 'border-[#25D366] bg-[#25D366]/10' : '',
            ...
            >
                ...
                <Check v-if="isSelected(contact.id)" class="h-5 w-5 text-[#25D366]" />
            </div>

            <div v-if="filteredContacts.length === 0" class="py-8 text-center text-muted-foreground">
