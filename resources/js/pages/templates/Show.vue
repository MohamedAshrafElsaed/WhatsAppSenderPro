...
import { destroy, edit, index } from '@/routes/dashboard/templates';
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    <Card>
                        <CardHeader>
                            <div :class="isRTL() ? 'flex-row-reverse' : ''" class="flex items-center justify-between">
                                <CardTitle :class="isRTL() ? 'text-right' : 'text-left'">
                                    {{ t('templates.template_details', 'Template Details') }}
                                </CardTitle>
                                <Badge :class="getTypeBadgeColor(template.type)" class="text-white">
                                    <component
                        ...
                        <CardContent class="space-y-4">
                            <!-- Content -->
                            <div>
                                <h4 :class="isRTL() ? 'text-right' : 'text-left'" class="mb-2 font-medium text-[#25D366]">
                                    {{ t('templates.template_content', 'Message Content') }}
                                </h4>
                                <div
                                    :class="isRTL() ? 'text-right' : 'text-left'"
                                    class="whitespace-pre-wrap rounded-lg border bg-muted/50 p-4"
                                >
                                    {{ template.content }}
                                </div>
                            </div>
                            ...
                            <div v-if="template.media_url">
                                ...
                                <div class="overflow-hidden rounded-lg border">
                                    ...
                                    />
                                    <video
                                        v-else-if="template.type === 'text_video'"
                                        :src="template.media_url"
                                        class="w-full"
                                        controls
                                    />
                                    <div
                                        v-else-if="template.type === 'text_document'"
                                        class="flex items-center gap-4 p-4 bg-orange-50 dark:bg-orange-950"
                                    >
                                        <FileText class="h-12 w-12 text-orange-500" />
                                        <div class="flex-1">
                                            <p class="font-medium">{{ t('templates.document_file', 'Document file') }}</p>
                                            <p class="text-sm text-muted-foreground">
                                                {{ template.media_url.split('/').pop() }}
                                            </p>
                                            <a
                                                :href="template.media_url"
                                                class="text-sm text-[#25D366] hover:underline font-medium"
                                                download
                                                target="_blank"
                                            >
                            ...
                            <div v-if="usedPlaceholders.length > 0">
                                ...
                                <h4 :class="isRTL() ? 'text-right' : 'text-left'" class="mb-2 font-medium text-[#25D366]">
                                    {{ t('templates.used_placeholders', 'Used Placeholders') }}
                                </h4>
                                <div class="flex flex-wrap gap-2">
                                    <Badge
                                        v-for="placeholder in usedPlaceholders"
                                        :key="placeholder.value"
                                        class="bg-[#25D366]/10 text-[#25D366] border-[#25D366]/20"
                                    >
                                        {{ placeholder.value }}
                                    </Badge>
                                </div>
                ...
                <div class="space-y-6">
                    ...
                    <Card class="border-[#25D366]/20">
                        ...
                        <CardContent class="space-y-4">
                            ...
                            <div :class="isRTL() ? 'flex-row-reverse text-right' : 'text-left'" class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm text-muted-foreground">
                                        {{ t('templates.usage_count', 'Usage Count') }}
                                    </p>
                                    <p class="text-xl font-semibold text-[#25D366]">
                    ...
                    <Card>
                        ...
                        <CardContent class="space-y-3 text-sm">
                            <div :class="isRTL() ? 'text-right' : 'text-left'">
                                <p class="text-muted-foreground">{{ t('common.created_at', 'Created At') }}</p>
                                <p class="font-medium">{{ formatDate(template.created_at) }}</p>
                            </div>
                            <Separator />
                            <div :class="isRTL() ? 'text-right' : 'text-left'">
                                <p class="text-muted-foreground">{{ t('common.updated_at', 'Updated At') }}</p>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle :class="isRTL() ? 'text-right' : 'text-left'">
                                {{ t('templates.available_placeholders', 'Available Placeholders') }}
                            </CardTitle>
                            <CardDescription :class="isRTL() ? 'text-right' : 'text-left'">
                                {{ t('templates.placeholders_info', 'You can use these placeholders in your templates') }}
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div class="space-y-2">
                                <div
                                    ...
                                    :class="isRTL() ? 'text-right' : 'text-left'"
                                >
                                    <Badge class="bg-[#25D366]/10 text-[#25D366] border-[#25D366]/20">
                                        {{ placeholder.value }}
                                    </Badge>
